...
.Steps
. [[first-step]]Record the cron job messages generated at the storage cutover to identify the job schedules that must be replicated to the remote site.
+
image::../media/mcc_cron_jobs.gif[]

. From the cluster CLI of the remote site, run the `job schedule cron create` commands recorded in <<first-step,the previous step>>.
